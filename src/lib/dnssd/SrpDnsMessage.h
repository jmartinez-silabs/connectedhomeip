 ...
 *
 *   This intentionally does not depend on the minimal-mDNS record model so it
 *   can be built on any platform that enables the SRP client or server,
 *   independent of the selected mDNS backend.
 */
...
/// KEY / SIG record RDATA constants (RFC 4034 / RFC 6605). Algorithm 13 is
/// ECDSA P-256 with SHA-256, which matches Matter's P256 crypto.
...
public:
    ...
    /// Write the fixed 12-byte header.
    CHIP_ERROR PutHeader(uint16_t id, uint16_t flags, uint16_t qdcount, uint16_t ancount, uint16_t nscount, uint16_t arcount);

    /// Overwrite the section counts in an already-written header.
    void PatchHeaderCounts(uint16_t qdcount, uint16_t ancount, uint16_t nscount, uint16_t arcount);

    /// Encode a dotted name (e.g. "foo._matter._tcp.default.service.arpa").
    /// An empty string or "." encodes the DNS root.
    ...
    CHIP_ERROR PutQuestion(const char * name, RecordType type, RecordClass klass);

    /// Begin a resource record: writes NAME, TYPE, CLASS, TTL and reserves the
    /// RDLENGTH field. Follow with rdata writes and then FinishRecord().
    CHIP_ERROR StartRecord(const char * name, RecordType type, RecordClass klass, uint32_t ttl);

    /// Backpatch the RDLENGTH of the record opened by StartRecord().
    CHIP_ERROR FinishRecord();

    // Typed record helpers (each is a full StartRecord()/rdata/FinishRecord()).
    CHIP_ERROR PutSrv(const char * name, RecordClass klass, uint32_t ttl, uint16_t priority, uint16_t weight, uint16_t port,
                      const char * target);
    CHIP_ERROR PutTxt(const char * name, RecordClass klass, uint32_t ttl, const TextEntry * entries, size_t count);
    CHIP_ERROR PutAaaa(const char * name, RecordClass klass, uint32_t ttl, const Inet::IPAddress & address);
    CHIP_ERROR PutPtr(const char * name, RecordClass klass, uint32_t ttl, const char * target);
    CHIP_ERROR PutKey(const char * name, RecordClass klass, uint32_t ttl, ByteSpan publicKey);

    /// RFC 2136 "delete an RRset" form: CLASS ANY, TTL 0, empty RDATA.
...
private:
    ...
    size_t mSize;
    size_t mOffset      = 0;
    size_t mRdlenOffset = 0; // offset of the RDLENGTH placeholder for the open record
    bool mInRecord      = false;
    bool mOk            = true;
};

/**
 * DNS message reader supporting name decompression.
 */
class DnsReader
{
public:
    ...
    /// Decode a (possibly compressed) name at the current offset into `out`.
    ...
    /// Skip a question / zone entry (NAME, TYPE, CLASS).
    CHIP_ERROR SkipQuestion();

    /// Read a resource-record header; leaves the offset at the start of RDATA.
    ...
    /// Parse a KEY record's RDATA (flags/protocol/algorithm + public key). The
    /// caller supplies the record's RDLENGTH so the key material is bounded.
