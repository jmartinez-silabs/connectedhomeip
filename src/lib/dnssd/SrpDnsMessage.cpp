...
CHIP_ERROR DnsWriter::PutHeader(uint16_t id, uint16_t flags, uint16_t qdcount, uint16_t ancount, uint16_t nscount,
                                uint16_t arcount)
{
...
{
    if (mSize < kDnsHeaderSize)
    {
        return;
    }
    WriteU16(mBuffer + 4, qdcount);
    WriteU16(mBuffer + 6, ancount);
    WriteU16(mBuffer + 8, nscount);
    WriteU16(mBuffer + 10, arcount);
}
...
CHIP_ERROR DnsWriter::PutQuestion(const char * name, RecordType type, RecordClass klass)
{
    ReturnErrorOnFailure(PutName(name));
    ReturnErrorOnFailure(PutU16(static_cast<uint16_t>(type)));
    ReturnErrorOnFailure(PutU16(static_cast<uint16_t>(klass)));
    return CHIP_NO_ERROR;
}

CHIP_ERROR DnsWriter::StartRecord(const char * name, RecordType type, RecordClass klass, uint32_t ttl)
{
...
CHIP_ERROR DnsWriter::FinishRecord()
{
    ...
    WriteU16(mBuffer + mRdlenOffset, static_cast<uint16_t>(rdlen));
    mInRecord = false;
    return CHIP_NO_ERROR;
}

CHIP_ERROR DnsWriter::PutSrv(const char * name, RecordClass klass, uint32_t ttl, uint16_t priority, uint16_t weight,
                            uint16_t port, const char * target)
{
    ReturnErrorOnFailure(StartRecord(name, RecordType::kSrv, klass, ttl));
...
CHIP_ERROR DnsWriter::PutTxt(const char * name, RecordClass klass, uint32_t ttl, const TextEntry * entries, size_t count)
{
    ReturnErrorOnFailure(StartRecord(name, RecordType::kTxt, klass, ttl));
...
CHIP_ERROR DnsWriter::PutAaaa(const char * name, RecordClass klass, uint32_t ttl, const Inet::IPAddress & address)
{
    ReturnErrorOnFailure(StartRecord(name, RecordType::kAaaa, klass, ttl));
...
CHIP_ERROR DnsWriter::PutPtr(const char * name, RecordClass klass, uint32_t ttl, const char * target)
{
    ReturnErrorOnFailure(StartRecord(name, RecordType::kPtr, klass, ttl));
    ReturnErrorOnFailure(PutName(target));
    return FinishRecord();
}

CHIP_ERROR DnsWriter::PutKey(const char * name, RecordClass klass, uint32_t ttl, ByteSpan publicKey)
{
    ...
    ReturnErrorOnFailure(StartRecord(name, RecordType::kKey, klass, ttl));
...
CHIP_ERROR DnsReader::SkipQuestion()
{
    ...
    ReturnErrorOnFailure(ReadU16(klass));
    return CHIP_NO_ERROR;
}
...
CHIP_ERROR DnsReader::ReadRecordHeader(RecordHeader & out)
{
    ...
    out.klass       = static_cast<RecordClass>(klass);
