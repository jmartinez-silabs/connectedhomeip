...
def get_supersets(xml_device_types: dict[int, XmlDeviceType]) -> list[set[int]]:
    ...
    top_level_device_types = [
        _id
        for _id, dt in xml_device_types.items()
...
class DeviceConformanceTests(BasicCompositionTests):
    ...
    def check_conformance(
        self,
        ignore_in_progress_test_event_only_disallowed_for_certification: bool,
        is_ci: bool,
        allow_provisional_test_event_only_disallowed_for_certification: bool,
    ):
        problems = []
        success = True
        ...
        if is_ci:
            ...
            ci_ignore_attributes = {
                Clusters.NetworkCommissioning.id: [
                    Clusters.NetworkCommissioning.Attributes.ScanMaxTimeSeconds.attribute_id,
                    Clusters.NetworkCommissioning.Attributes.ConnectMaxTimeSeconds.attribute_id,
                ]
            }
            ...
            for cluster_id, cluster in endpoint.items():
                ...
                if cluster_id not in self.xml_clusters:
                    if (cluster_id & 0xFFFF_0000) != 0:
                        ...
                        continue
                    record_error(
                        location=cluster_location, problem="Standard cluster found on device, but is not present in spec data"
                    )
                    continue
                ...
                location = AttributePathLocation(
                    endpoint_id=endpoint_id, cluster_id=cluster_id, attribute_id=GlobalAttributeIds.FEATURE_MAP_ID
                )
                ...
                for f in feature_masks:
                    ...
                    if cluster_id == Clusters.AccessControl.id and f == Clusters.AccessControl.Bitmaps.Feature.kManagedDevice:
                        ...
                        if not self._has_device_type_supporting_macl():
                            record_error(
                                location=location,
                                problem="MACL feature is disallowed if the a supported device type is not present",
                            )
                        continue

                    if f not in self.xml_clusters[cluster_id].features:
                        record_error(
                            location=location, problem=f"Unknown feature with mask 0x{f:02x} (feature bit {f.bit_length() - 1})"
                        )
                        continue
                    xml_feature = self.xml_clusters[cluster_id].features[f]
                    conformance_decision_with_choice = xml_feature.conformance(cluster_info)
                    if not conformance_allowed(
                        conformance_decision_with_choice, allow_provisional_test_event_only_disallowed_for_certification
                    ):
                        record_error(
                            location=location, problem=f"Disallowed feature with mask 0x{f:02x} (feature bit {f.bit_length() - 1})"
                        )
                for feature_mask, xml_feature in self.xml_clusters[cluster_id].features.items():
                    ...
                    if conformance_decision_with_choice.is_mandatory() and feature_mask not in feature_masks:
                        record_error(
                            location=location,
                            problem=f"Required feature with mask 0x{feature_mask:02x} (feature bit {feature_mask.bit_length() - 1}) is not present in feature map. {conformance_str(xml_feature.conformance, feature_map, self.xml_clusters[cluster_id].features)}",
                        )
                ...
                def check_spec_conformance_for_commands(command_type: CommandType):
                    global_attribute_id = (
                        ...
                        else GlobalAttributeIds.GENERATED_COMMAND_LIST_ID
                    )
                    xml_commands_dict = (
                        self.xml_clusters[cluster_id].accepted_commands
                        if command_type == CommandType.ACCEPTED
                        else self.xml_clusters[cluster_id].generated_commands
                    )
        ...
        for endpoint_id, endpoint in self.endpoints_tlv.items():
            for cluster_id, cluster in endpoint.items():
                ...
                if int(self.xml_clusters[cluster_id].revision) != cluster[GlobalAttributeIds.CLUSTER_REVISION_ID]:
                    location = AttributePathLocation(
                        endpoint_id=endpoint_id, cluster_id=cluster_id, attribute_id=GlobalAttributeIds.CLUSTER_REVISION_ID
                    )
                    record_error(
                        location=location,
                        problem=f"Revision found on cluster ({cluster[GlobalAttributeIds.CLUSTER_REVISION_ID]}) does not match revision listed in the spec ({self.xml_clusters[cluster_id].revision})",
                    )
    ...
    def check_device_type_revisions(self) -> tuple[bool, list[ProblemNotice]]:
        ...
        for endpoint_id, endpoint in self.endpoints.items():
            ...
            standard_device_types = [
                x
                for x in endpoint[Clusters.Descriptor][Clusters.Descriptor.Attributes.DeviceTypeList]
                if device_type_id_type(x.deviceType) == DeviceTypeIdType.kStandard
            ]
            for device_type in standard_device_types:
                ...
                if expected_revision != actual_revision:
                    ...
                    record_error(
                        location,
                        f"Expected Device type revision for device type {device_type_id} {self.xml_device_types[device_type_id].name} on endpoint {endpoint_id} does not match revision on DUT. Expected: {expected_revision} DUT: {actual_revision}",
                    )
        return success, problems

    def check_device_type(
        self, fail_on_extra_clusters: bool = True, allow_provisional_test_event_only_disallowed_for_certification: bool = False
    ) -> tuple[bool, list[ProblemNotice]]:
        success = True
        problems = []
        ...
        for endpoint_id, endpoint in self.endpoints.items():
            ...
            for device_type in standard_device_types:
                ...
                server_clusters = [
                    x
                    for x in endpoint[Clusters.Descriptor][Clusters.Descriptor.Attributes.ServerList]
                    if cluster_id_type(x) == ClusterIdType.kStandard
                ]
                ...
                for cluster_id, cluster_requirement in xml_device.server_clusters.items():
                    ...
                    if cluster_id in server_clusters and not conformance_allowed(
                        conformance_decision_with_choice, allow_provisional_test_event_only_disallowed_for_certification
                    ):
                        record_error(
                            location=location,
                    ...
                    def check_feature_overrides(
                        cluster_requirement: XmlDeviceTypeClusterRequirements, cluster_info: ConformanceAssessmentData
                    ):
                        for mask, conformance in cluster_requirement.feature_overrides.items():
                            ...
                            if conformance_decision_with_choice.is_mandatory() and ((feature_map & mask) == 0):
                                record_error(
                                    location=location,
                                    problem=f"Feature bit {mask.bit_length() - 1} in cluster {cluster_requirement.name} is required by element override for device type {xml_device.name}, but is not present in the feature map",
                                )
                            if not conformance_allowed(
                                conformance_decision_with_choice, allow_provisional_test_event_only_disallowed_for_certification
                            ) and ((feature_map & mask) != 0):
                                record_error(
                                    location=location,
                                    problem=f"Feature bit {mask.bit_length() - 1} in cluster {cluster_requirement.name} is disallowed by element override for device type {xml_device.name}, but is present in the feature map",
                                )

                    def check_attribute_overrides(
                        cluster_requirement: XmlDeviceTypeClusterRequirements, cluster_info: ConformanceAssessmentData
                    ) -> None:
                        for _id, conformance in cluster_requirement.attribute_overrides.items():
                            ...
                            if conformance_decision_with_choice.is_mandatory() and _id not in attribute_list:
                                record_error(
                                    location=location,
                                    problem=f"Attribute {_id} in cluster {cluster_requirement.name} is required by element override for device type {xml_device.name}, but is not present in the attribute list",
                                )
                            if (
                                not conformance_allowed(
                                    conformance_decision_with_choice, allow_provisional_test_event_only_disallowed_for_certification
                                )
                                and _id in attribute_list
                            ):
                                if (
                                    device_type_id == water_heater_id
                                    and cluster_id == Clusters.Thermostat.id
                                    and _id == Clusters.Thermostat.Attributes.SystemMode.attribute_id
                                ):
                                ...
                                record_error(
                                    location=location,
                                    problem=f"Attribute {_id} in cluster {cluster_requirement.name} is disallowed by element override for device type {xml_device.name}, but is present in the attribute list",
                                )

                    def check_command_overrides(
                        cluster_requirement: XmlDeviceTypeClusterRequirements, cluster_info: ConformanceAssessmentData
                    ):
                        for _id, conformance in cluster_requirement.command_overrides.items():
                            ...
                            if conformance_decision_with_choice.is_mandatory() and _id not in cmd_list:
                                record_error(
                                    location=location,
                                    problem=f"Command {_id} in cluster {cluster_requirement.name} is required by element override for device type {xml_device.name}, but is not present in the cmd list",
                                )
                            if (
                                not conformance_allowed(
                                    conformance_decision_with_choice, allow_provisional_test_event_only_disallowed_for_certification
                                ...
                                record_error(
                                    location=location,
                                    problem=f"Command {_id} in cluster {cluster_requirement.name} is disallowed by element override for device type {xml_device.name}, but is present in the cmd list",
                                )
    ...
    def check_all_application_device_types_superset(self) -> list[ProblemNotice]:
        ...
        for endpoint_num, endpoint in self.endpoints.items():
            all_device_type_ids = [
                dt.deviceType for dt in endpoint[Clusters.Descriptor][Clusters.Descriptor.Attributes.DeviceTypeList]
            ]
            ...
            for cluster in endpoint:
                if cluster in root_node_restricted_clusters:
                    problems.append(
                        ProblemNotice(
                            "TC-IDM-14.1",
                            location=ClusterPathLocation(endpoint_id=endpoint_id, cluster_id=cluster.id),
                            severity=ProblemSeverity.ERROR,
                            problem=f"Root-node-restricted cluster {cluster} appears on non-root-node endpoint",
                        )
                    )
        return problems
        ...
        for endpoint_id, endpoint in self.endpoints.items():
            ...
            if have_closure and have_window_covering:
                for dt in device_types:
                    ...
                    if (
                        device_type_id in restricted_device_type_ids
                        and dt.revision <= one_five_device_types[device_type_id].revision
                    ):
                        problems.append(
                            ProblemNotice(
                                "TC-IDM-14.1",
    ...
    def check_closure_restricted_sem_tags(self) -> list[ProblemNotice]:
        ...
        def check_tags_on_endpoint(
            device_type_id: int, allowed_namespace: int, max_num_tags_allowed_namespace: int, disallowed_namespace: int
        ):
        ...
        for endpoint_id, endpoint in self.endpoints.items():
            ...
            for dt in device_types:
                if dt.deviceType == closure_id:
                    check_tags_on_endpoint(
                        device_type_id=closure_id,
                        allowed_namespace=closure_namespace_id,
                        max_num_tags_allowed_namespace=1,
                        disallowed_namespace=closure_panel_namespace_id,
                    )
                elif dt.deviceType == closure_panel_id:
                    check_tags_on_endpoint(
                        device_type_id=closure_panel_id,
                        allowed_namespace=closure_panel_namespace_id,
                        max_num_tags_allowed_namespace=1,
                        disallowed_namespace=closure_namespace_id,
                    )
